...
5. If you want to keep local edits, run the migration below **before opening the site or admin with the remote connection**. Otherwise restart locally or deploy, then log into /admin and verify an edit and image upload. On its first connection an empty remote catalog receives the original six products and three categories. The private local preview catalog is not automatically copied to the remote project.
